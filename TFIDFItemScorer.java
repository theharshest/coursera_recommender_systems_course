...
public class TFIDFItemScorer extends AbstractItemScorer {
    ...
    public void score(long user, @Nonnull MutableSparseVector output) {
        ...
        for (VectorEntry e: output.fast(VectorEntry.State.EITHER)) {
            ...
            SparseVector usr = makeUserVector(user);
            double sim = usr.dot(iv)/(usr.norm()*iv.norm());
            output.set(e.getKey(), sim);
    ...
    private SparseVector makeUserVector(long user) {
        ...
        for (Rating r: userRatings) {
            s = s + r.getPreference().getValue();
        }

        s = s/(double)userRatings.size();
        ...
        for (Rating r: userRatings) {
            ...
            if (p != null) {
                ...
                MutableSparseVector tmp = model.getItemVector(p.getItemId()).mutableCopy();
                tmp.multiply(p.getValue()-s);
